...
function calendar($month, $year, $games){
	...
	for($list_day = 1; $list_day <= $days_in_month; $list_day++):
		...
		$calendar.= '<td class="calendar-day" id="cell'.$games[$key]['lid'].'" style="background-color: '.$color.'; color:black; font-weight: bold; font-size: 15px;">';
			...
			$calendar.= '<div  class="day-number" style="font-size: 12px; font-weight: normal">'.
                        ...
                        if($key){
                           $calendar.= ($games[$key]['sold'] != "2") ? '<input type="checkbox" style="display:none" class="checkbox" name="'.$games[$key]['lid'].'" id="'.$games[$key]['lid'].'"/>' : "";
                           $calendar .= ($games[$key]['sold'] != "2") ? '<label for="'.$games[$key]['lid'].'">' : "<div class='sold'>";
                           $calendar .= $games[$key]['title'];
                           $calendar .= ($games[$key]['sold'] != "2") ?  '</label>' : "</div>";
                           $calendar .= ($games[$key]['sold'] != "2") ? "<span class='status' onclick=\"status('a','".$games[$key]['lid']."')\">Change Status</span>" : "";
